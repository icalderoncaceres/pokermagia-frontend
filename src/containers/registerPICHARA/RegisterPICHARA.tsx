...
function RegisterPICHARA() {
    ...
    const loadDataFromServer = async (day: number, month: number) => {
        let response = await get({ userId: user.id, room: 'pichara', day, month });
        if (response.data) {
            setData({
    ...
    const loadMonthlyDataFromServer = async (month: number) => {
        let response = await getMonthly({ userId: user.id, month, room: 'pichara' });
    ...
    const handleSave = useCallback((month: number) => {
        (async () => {
            try {
                const response = await saveMonthly({ userId: user.id, room: 'pichara', month, data: monthlyData });
...
export default RegisterPICHARA;